...
## 🔴 إصلاح حرج: الترجمة كانت معطّلة كليًا في المتصفح

**التاريخ:** 2026-08-28

اكتُشف بتشغيل التطبيق فعليًا واختباره — لا باختبارات الوحدة (كانت 248 اختبارًا تمرّ
بينما المنتج لا يترجم).

### الخلل الأساسي

- **`server/routes-sse.js` لم يُركَّب في `server.js` قط** (منذ الموجة 3، الالتزام `661e00f`).
  `POST /api/translate-stream` كان يُرجع **404**، والواجهة تعتمده كمسارها الافتراضي
  (`supportsStreaming()` صحيحة في كل متصفح حديث) بلا سقوط احتياطي ⇒ وضعا «ترجمة رابط»
  و«ترجمة نص» يفشلان دائمًا.
- **موضع التركيب حرج**: `'text/event-stream'` نوع قابل للضغط، فتركيب الراوتر بعد
  `compression()` كان سيخزّن الأحداث مؤقتًا وتصل الترجمة دفعة واحدة — يعمل الطلب
  ويضيع البثّ صامتًا. رُكِّب بين `express.json()` و`compression()` عمدًا، ومعه اختبار حارس.
- **خلل ثانٍ في نفس المسار**: كشف انقطاع العميل كان على `req.on('close')`، ومنذ Node 16
  يُطلق هذا الحدث فور اكتمال قراءة جسم الطلب — و`express.json` يستهلكه قبل المعالج،
  فكان `disconnected=true` دائمًا وكل حدث يُهمَل والبثّ يعود **فارغًا**. صار على `res`.
- **وصول وقت التنفيذ**: `routes-sse` كان يفكّك `fetchArticleContent`/`detectLanguage`
  وقت الاستيراد فيتجمّد المرجع ويتعذّر تزييفه (نفس الخلل المعالَج في `routes-translate.js`
  بالالتزام `efd81ed`).

### الواجهة

- `public/js/translate.js`: استُخرج المسار التقليدي إلى `runClassic()`، ويُستدعى تلقائيًا
  عند `stream-failed`/`network-error` — فلا يكسر عطلٌ في البثّ وحده وظيفةَ المنتج مجددًا.

### خلل ثالث: تصنيف السياق

- `server/context-detect.js`: نمط كشف الكود كان بلا حدود كلمات، فتطابق الكلمات المحجوزة
  داخل كلمات إنجليزية عادية — `const` في "co**nst**ant"، `import` في "**import**ant"،
  `let` في "comp**let**ely"، `def` في "**def**endant". النتيجة: نصوص قانونية وطبية شائعة
  تُصنَّف «كودًا» فتأخذ توجيه ترجمة خاطئًا. أُضيفت حدود الكلمات في النمط وفي مطابقة
  الكلمات المفتاحية (`matchesKeyword`).

### الاختبارات — سدّ السبب الجذري

لم يمسك أيٌّ من الـ248 اختبارًا شيئًا من هذا لأن لا اختبار كان يصيب المسار ولا يتحقق
من أن التطبيق **مُركَّب كما تتوقعه الواجهة**:

- **`tests/smoke.test.js`** (17) — كل مسار حرج ليس 404، وحارس آلي يستخرج مسارات `/api`
  من `public/js/*.js` ويطابقها بمسارات Express المسجَّلة عبر التأمّل. مُثبَت: إزالة تركيب
  SSE تُفشل اختبارين فورًا.
- **`tests/sse.test.js`** (8) — تسلسل `init → chunk* → done`، وحارس صريح على غياب
  `Content-Encoding` (يمنع عودة الراوتر خلف `compression`).
- **`tests/ssrf.test.js`** (34) — أخطر وحدة وكانت بلا تغطية: نطاقات IPv4/IPv6 الداخلية،
  بيانات السحابة الوصفية، صيغ الالتفاف العشرية والثمانية، DNS rebinding، الإغلاق الآمن.
- **`tests/contextDetect.test.js`** (19) — كشفت خلل التصنيف أعلاه.
- **`tests/apiKey.test.js`** — قُلب ليؤكّد **رفض** `?api_key=` (المفتاح في الاستعلام يظهر
  في السجلات وReferrer)؛ ووُثّق ذلك في `.env.example`.
- **`tests/loggerLanguages.test.js`** (10) — آخر وحدتين بلا تغطية: تدوير السجل واقتطاع
  الرسائل وعدم الرمي أبدًا؛ وشكل `getAllLanguages()` كمصفوفة `{code,nameAr}` — وهو
  الشكل الذي أخطأت الواجهة في قراءته سابقًا. **كل وحدات الخادم مغطّاة الآن.**

## 🗄️ طبقة تخزين عدّادات حد الطلبات — المرحلة 2

**التاريخ:** 2026-08-28

- **`server/store.js`** جديد: واجهة مفتاح/قيمة قابلة للتبديل لعدّادات حد الطلبات.
  - **مُشغّل الذاكرة** (الافتراضي) — سلوك مطابق تمامًا للسابق (نسخة خادم واحدة).
  - **مُشغّل Redis** — يُفعَّل بضبط `REDIS_URL`؛ يشارك العدّادات بين نسخ الخادم
    (`INCR` + `PEXPIRE`). بدونه كان الحد الفعلي = `RATE_LIMIT_MAX` × عدد النسخ.
  - **سقوط آمن**: الحزمة `redis` اختيارية؛ أي فشل (غير مثبّتة/تعذّر اتصال/خطأ
    وقت تشغيل) → تحذير مرة واحدة + تحوّل دائم للذاكرة + السماح بالطلب (fail-open).
    تسخين الاتصال عند الإقلاع فيقع السقوط قبل أول طلب مستخدم (مهلة اتصال 3s).
- **`server/server.js`**: `createRateLimiter` صار غير متزامن ويمرّ عبر المتجر؛
  إغلاق اتصال Redis المشترك على `SIGINT`/`SIGTERM`.
- **اختبارات**: `tests/store.test.js` (6) — مُشغّل الذاكرة بلا شبكة: العدّ، عزل
  المفاتيح/المتاجر، انتهاء النافذة، `reset`.
- `package.json`: `redis` في `optionalDependencies`.
- الكاش (`cache.js`) والإحصائيات تبقى ملفات — مناسبة لنسخة واحدة (نقلها إلى Redis
  خطوة لاحقة موثّقة).
